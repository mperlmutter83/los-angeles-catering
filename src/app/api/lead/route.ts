...
const FROM = 'Los Angeles Catering and Bar <info@yesidoinc.com>';
...
async function notifyWorkApplicant(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;font-size:15px;color:#111;">
      <h2 style="margin:0 0 12px;">New work applicant — losangelescateringandbar.com</h2>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `New work applicant — ${opts.name} (losangelescateringandbar.com)`,
        html,
      }),
    });
    if (!res.ok) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, phone, service, message, submission_id, elapsed_ms, company_website } =
      body;
    ...
    const serviceNeeded = [
      ...
      'Source: losangelescateringandbar.com',
    ]
      .filter(Boolean)
      .join(' | ');
